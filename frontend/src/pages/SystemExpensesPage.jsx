...
const SystemExpensesPage = () => {
  ...
  const calculateStats = (expensesData) => {
    ...
    // Site filtering
    if (siteFilter !== 'all') {
      timeFilteredExpenses = timeFilteredExpenses.filter(exp => exp.site === siteFilter);
    }
    ...
    const monthlyAmount = expensesData.filter(exp => {
      const expDate = new Date(exp.expenseDate);
      return expDate.getMonth() === currentMonth && expDate.getFullYear() === currentYear;
    ...
    const siteMap = {};
    timeFilteredExpenses.forEach(exp => {
      const siteName = exp.site?.name || 'No Site';
      if (siteMap[siteName]) {
        siteMap[siteName].count += 1;
        siteMap[siteName].amount += exp.amount;
      } else {
        siteMap[siteName] = { count: 1, amount: exp.amount };
      }
    });

    const siteBreakdown = Object.entries(siteMap).map(([site, data]) => ({
      site,
      count: data.count,
      amount: data.amount,
      percentage: (data.count / totalExpenses) * 100
  ...
  }, [timeRange, categoryFilter, siteFilter, searchTerm, expenses]);
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'submitted': return <Assessment />;
      case 'under_review': return <Speed />;
        ...
        <Fade in timeout={600}>
          <Box sx={{ mb: 4 }}>
            ...
            <Typography variant="body1" color={darkMode ? '#b0b0b0' : '#666'}>
              Comprehensive analysis of all system-wide expenses and spending patterns
            </Typography>
          </Box>
        </Fade>
                ...
                <FormControl fullWidth>
                  ...
                  >
                    ...
                    {stats.siteBreakdown.map((site) => (
                      <MenuItem key={site.site} value={site.site}>
                        {site.site}
                      </MenuItem>
                    ))}
                  </Select>
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                  ...
                  <Button
                    ...
                    onClick={createCSVExportHandler(
                      { 
                        summary: { totalSystemExpenses: expenses.length, totalAmount: stats.totalAmount },
                        expenses: expenses,
                  ...
                  {stats.siteBreakdown.map((site, index) => (
                    <Grid item xs={12} key={site.site}>
                      <Card sx={{ 
                        ...
                        <CardContent>
                          <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                            <Business sx={{ fontSize: 20, mr: 1, color: COLORS[index % COLORS.length] }} />
                            <Typography variant="h6" fontWeight={600} color={darkMode ? '#fff' : '#333'}>
                              {site.site}
                            </Typography>
                          </Box>
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                            ...
                            <Typography variant="body2" color={darkMode ? '#b0b0b0' : '#666'}>
                              Amount: ₹{site.amount.toLocaleString()}
                            </Typography>
                          </Box>
                          <Typography variant="body2" color={darkMode ? '#b0b0b0' : '#666'} sx={{ mb: 1 }}>
                            {site.percentage.toFixed(1)}% of total
                          </Typography>
                          <LinearProgress 
                            variant="determinate" 
...
export default SystemExpensesPage;
