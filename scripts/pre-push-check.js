...
// Check 2: Verify .env.production.encrypted files exist and are up to date
for (const envFile of envFiles) {
    const encryptedFile = envFile + '.encrypted';
    
    if (!fs.existsSync(encryptedFile)) {
        console.error(`❌ Error: ${encryptedFile} file is missing!`);
        console.error(`Please run: node scripts/env-manager.js encrypt ${envFile}`);
        process.exit(1);
    }
    console.log(`✅ ${encryptedFile} file exists`);
}
...
// Check 4: Ensure .env.production files are not tracked by git
try {
    ...
    for (const line of statusLines) {
        ...
        for (const envFile of envFiles) {
            if (filename === envFile) {
                ...
                console.error(`1. Add ${envFile} to .gitignore`);
                console.error(`2. Remove it from git: git rm --cached ${envFile}`);
                console.error('3. Commit the changes');
                process.exit(1);
            }
        }
    }
    console.log('✅ .env.production files are not tracked by git');
} catch (error) {
...
// Check 9: Ensure encrypted files are newer than source files
for (const envFile of envFiles) {
    if (fs.existsSync(envFile)) {
        const envStats = fs.statSync(envFile);
        const encryptedStats = fs.statSync(envFile + '.encrypted');
        
        if (envStats.mtime > encryptedStats.mtime) {
            console.error(`❌ Error: ${envFile} is newer than ${envFile}.encrypted!`);
            console.error(`Please run: node scripts/env-manager.js encrypt ${envFile}`);
            process.exit(1);
        }
        console.log(`✅ ${envFile}.encrypted is up to date`);
    }
}
